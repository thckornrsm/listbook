...
<template>
  <div class="relative">
    ...
    >
      ...
      >
        <!-- มีผล -->
        <ul v-if="suggestions.length > 0" class="max-h-60 overflow-y-auto overflow-x-hidden py-1">
          <li
          ...
          >
            <!-- บรรทัด 1: ตำบล (ไฮไลต์) -->
            <div class="font-semibold text-slate-800" v-html="highlight(item.subdistrict)" />
            <!-- บรรทัด 2: อำเภอ · จังหวัด · ไปรษณีย์ -->
...
const { searchBySubdistrict } = useThaiAddress()
...
// sync จาก parent (กรณี reset form)
watch(
  () => props.modelValue,
  (val) => {
...
const selectHighlighted = () => {
  ...
  if (highlightedIndex.value >= 0 && item !== undefined) {
    selectItem(item)
  }
}
...
const moveDown = () => {
  ...
  if (highlightedIndex.value < suggestions.value.length - 1) highlightedIndex.value++
}
