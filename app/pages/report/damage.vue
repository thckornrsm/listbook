<template>
  <div class="min-h-dvh bg-slate-100 ">
    ...
    <div class="sticky top-0 z-20 flex h-14 items-center bg-[#ED7E24] px-4 text-white">
      ...
      <div class="flex-1  text-base font-semibold">แจ้งชำรุด/สูญหาย</div>
    ...
    <div class="mx-auto w-full max-w-md px-4 py-4">
      ...
      <div class="rounded-2xl bg-white p-4 shadow-sm">
        ...
        <div class="mt-1 flex h-12 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4">
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" class="shrink-0 text-slate-400">
          ...
          <input
            v-model="q"
            class="h-full flex-1 bg-transparent text-sm  placeholder:text-slate-400 focus:outline-none"
            placeholder="ระบุรหัสสินค้า หรือ สแกน"
          />

          <button type="button" class="grid h-9 w-9 shrink-0 place-items-center rounded-lg active:opacity-80" @click="onScan" aria-label="Scan">
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none">
              <path d="M9 4H6a2 2 0 0 0-2 2v3" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" />
              <path d="M15 4h3a2 2 0 0 1 2 2v3" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" />
              <path d="M9 20H6a2 2 0 0 1-2-2v-3" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" />
              <path d="M15 20h3a2 2 0 0 0 2-2v-3" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" />
              <path d="M9 9h6v6H9z" stroke="#94A3B8" stroke-width="2" />
            </svg>
          </button>
        </div>

        <!-- ✅ Dropdown ใช้ USelect Nuxt UI v3 -->
        <div class="mt-4">
          <div class="text-base font-semibold">เลือกหัวข้อปัญหา</div>
    ...
    <!-- Success Modal -->
    <UModal v-model:open="open" :ui="modalUi">
      <template #content>
          ...
          <div class="flex justify-center">
            <div class="grid h-28 w-28 place-items-center rounded-full bg-[#FDEFE5]">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 82 82" class="h-14 w-14" fill="none">
                <path d="M75.1668 37.8566V40.9999C75.1626 48.3677 72.7769 55.5368 68.3654 61.4378C63.9539 67.3389 57.753 71.6559 50.6876 73.7449C43.6222 75.834 36.0708 75.5831 29.1596 73.0298C22.2484 70.4764 16.3477 65.7575 12.3376 59.5766C8.32749 53.3957 6.42278 46.0842 6.90756 38.7324C7.39233 31.3805 10.2406 24.3824 15.0276 18.7816C19.8146 13.1808 26.2838 9.2775 33.4705 7.65378C40.6571 6.03005 48.1761 6.77293 54.906 9.77161" stroke="#ED7E24" stroke-width="7" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M75.1667 13.6667L41 47.8676L30.75 37.6176" stroke="#ED7E24" stroke-width="7" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </div>
          </div>

          <h3 class="mt-5 text-center text-2xl ">แจ้งชำรุดเรียบร้อย</h3>
          <p class="mt-2 text-center text-sm leading-6 text-[#62748E]">ระบบได้บันทึกข้อมูลและส่งแจ้งเตือนไปยังเจ้าหน้าที่แล้ว</p>

          <div class="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm">
            <div class="flex items-center justify-between">
              <div class="text-[#62748E]">Ticket ID :</div>
              <div class="">{{ successTicketId }}</div>
            </div>
            <div class="mt-2 flex items-center justify-between">
              <div class="text-[#62748E]">สถานะ :</div>
              <div class="text-[#ED7E24]">{{ successStatus }}</div>
            </div>
          </div>
...
const modalUi = {
  ...
  content: 'w-full max-w-sm rounded-2xl p-0 shadow-xl ring-0'
}
...
// ✅ ใช้ items แทน options สำหรับ Nuxt UI v3
const topicOptions = [
  { label: 'อุปกรณ์ชำรุด', value: 'damage' },
  { label: 'อุปกรณ์สูญหาย', value: 'lost' },
  { label: 'อุปกรณ์/อะไหล่ไม่ครบ', value: 'missing_parts' },
  { label: 'อื่นๆ', value: 'other' }
]

const canSubmit = computed(() => {
  return topic.value.trim().length > 0 && detail.value.trim().length > 0
})
